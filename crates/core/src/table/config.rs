...
mod tests {
    ...
    #[test]
    fn parquet_cdc_options_disabled_by_default() {
        use std::collections::HashMap;
        assert!(parquet_cdc_options(&HashMap::new(), &HashMap::new()).is_none());
    }

    #[test]
    fn parquet_cdc_options_uses_snapshot_over_write_time() {
        use std::collections::HashMap;
        let snapshot = HashMap::from([(PARQUET_CDC_ENABLED.to_string(), "true".to_string())]);
        let write_time =
            HashMap::from([(PARQUET_CDC_MIN_CHUNK_SIZE.to_string(), "1024".to_string())]);

        let cdc = parquet_cdc_options(&snapshot, &write_time).unwrap();
